...
describe 'compiled component ecs-service' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/extra_hosts.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/extra_hosts/ecs-service.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Task" do
      ...
      it "to have property ContainerDefinitions" do
          expect(resource["Properties"]["ContainerDefinitions"]).to eq([{"Name"=>"nginx", "Image"=>{"Fn::Join"=>["", ["nginx/", "nginx", ":", "latest"]]}, "LogConfiguration"=>{"LogDriver"=>"awslogs", "Options"=>{"awslogs-group"=>{"Ref"=>"LogGroup"}, "awslogs-region"=>{"Ref"=>"AWS::Region"}, "awslogs-stream-prefix"=>"nginx"}}, "ExtraHosts"=>[{"Hostname"=>"example.com", "IpAddress"=>"127.0.0.1"}, {"Hostname"=>"example2.com", "IpAddress"=>"127.0.0.2"}]}])
      end
