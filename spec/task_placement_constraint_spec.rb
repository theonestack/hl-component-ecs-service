...
describe 'compiled component ecs-service' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/task_placement_constraint.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/task_placement_constraint/ecs-service.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Task" do
      ...
      it "to have property PlacementConstraints" do
          expect(resource["Properties"]["PlacementConstraints"]).to eq([{"Type"=>"memberOf", "Expression"=>{"Fn::Sub"=>"attribute:ecs.availability-zone==ap-southeast-2a"}}])
      end
