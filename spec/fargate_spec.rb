...
describe 'compiled component ecs-service' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/fargate/ecs-service.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Service" do
      ...
      it "to have property PlatformVersion" do
          expect(resource["Properties"]["PlatformVersion"]).to eq({"Ref"=>"PlatformVersion"})
      end
      ...
      it "to have property NetworkConfiguration" do
          expect(resource["Properties"]["NetworkConfiguration"]).to eq({"AwsvpcConfiguration"=>{"AssignPublicIp"=>"DISABLED", "SecurityGroups"=>[{"Ref"=>"SecurityGroupBackplane"}], "Subnets"=>{"Ref"=>"SubnetIds"}}})
      end
