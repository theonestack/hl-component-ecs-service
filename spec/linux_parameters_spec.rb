...
describe 'compiled component ecs-service' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/linux_parameters.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/linux_parameters/ecs-service.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Task" do
      ...
      it "to have property ContainerDefinitions" do
          expect(resource["Properties"]["ContainerDefinitions"]).to eq([{"Name"=>"one", "Image"=>{"Fn::Join"=>["", ["nginx/", "nginx", ":", "latest"]]}, "LogConfiguration"=>{"LogDriver"=>"awslogs", "Options"=>{"awslogs-group"=>{"Ref"=>"LogGroup"}, "awslogs-region"=>{"Ref"=>"AWS::Region"}, "awslogs-stream-prefix"=>"one"}}, "LinuxParameters"=>{"Capabilities"=>{"Add"=>["ALL"], "Drop"=>["MKNOD"]}, "InitProcessEnabled"=>true, "MaxSwap"=>10, "SharedMemorySize"=>10, "Swappiness"=>10}}, {"Name"=>"two", "Image"=>{"Fn::Join"=>["", ["nginx/", "nginx", ":", "latest"]]}, "LogConfiguration"=>{"LogDriver"=>"awslogs", "Options"=>{"awslogs-group"=>{"Ref"=>"LogGroup"}, "awslogs-region"=>{"Ref"=>"AWS::Region"}, "awslogs-stream-prefix"=>"two"}}, "LinuxParameters"=>{"Capabilities"=>{"Drop"=>["CHOWN"]}}}])
      end
