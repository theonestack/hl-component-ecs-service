...
describe 'compiled component ecs-service' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/scheduling_strategy_daemon.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/scheduling_strategy_daemon/ecs-service.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Service" do
      ...
      it "to have property SchedulingStrategy" do
          expect(resource["Properties"]["SchedulingStrategy"]).to eq("DAEMON")
      end
