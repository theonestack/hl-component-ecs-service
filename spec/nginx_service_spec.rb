...
describe 'compiled component ecs-service' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/nginx_service.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/nginx_service/ecs-service.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Service" do
      ...
      it "to have property HealthCheckGracePeriodSeconds" do
          expect(resource["Properties"]["HealthCheckGracePeriodSeconds"]).to eq(60)
      end
