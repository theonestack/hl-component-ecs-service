...
describe 'compiled component ecs-service' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/multiple_links.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/multiple_links/ecs-service.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "Task" do
      ...
      it "to have property ContainerDefinitions" do
          expect(resource["Properties"]["ContainerDefinitions"]).to eq([{"Name"=>"nginx", "Image"=>{"Fn::Join"=>["", ["nginx/", "nginx", ":", "latest"]]}, "LogConfiguration"=>{"LogDriver"=>"awslogs", "Options"=>{"awslogs-group"=>{"Ref"=>"LogGroup"}, "awslogs-region"=>{"Ref"=>"AWS::Region"}, "awslogs-stream-prefix"=>"nginx"}}}, {"Name"=>"db", "Image"=>{"Fn::Join"=>["", ["", "postgres", ":", "latest"]]}, "LogConfiguration"=>{"LogDriver"=>"awslogs", "Options"=>{"awslogs-group"=>{"Ref"=>"LogGroup"}, "awslogs-region"=>{"Ref"=>"AWS::Region"}, "awslogs-stream-prefix"=>"db"}}, "PortMappings"=>[{"ContainerPort"=>5432}]}, {"Name"=>"redis", "Image"=>{"Fn::Join"=>["", ["", "redis", ":", "latest"]]}, "LogConfiguration"=>{"LogDriver"=>"awslogs", "Options"=>{"awslogs-group"=>{"Ref"=>"LogGroup"}, "awslogs-region"=>{"Ref"=>"AWS::Region"}, "awslogs-stream-prefix"=>"redis"}}, "PortMappings"=>[{"ContainerPort"=>6379}]}, {"Name"=>"php-fpm", "Image"=>{"Fn::Join"=>["", ["", "php-fpm", ":", "latest"]]}, "LogConfiguration"=>{"LogDriver"=>"awslogs", "Options"=>{"awslogs-group"=>{"Ref"=>"LogGroup"}, "awslogs-region"=>{"Ref"=>"AWS::Region"}, "awslogs-stream-prefix"=>"php-fpm"}}, "Links"=>["db:db", "redis:redis", "nginx:nginx"], "VolumesFrom"=>[{"SourceContainer"=>"db"}, {"SourceContainer"=>"redis"}], "PortMappings"=>[{"ContainerPort"=>9000}, {"ContainerPort"=>9001}]}])
      end
